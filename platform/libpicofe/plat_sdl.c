...
int plat_sdl_change_video_mode(int w, int h, int force)
{
  ...
  if (plat_target.vout_method != 0) {
    ...
    if (ScreenSurface == NULL) {
      fprintf(stderr, "SDL_SetVideoMode failed(ScreenSurface): %s\n", SDL_GetError());
      return -1;
    }
    ...
    if (plat_sdl_screen == NULL) {
      ...
      plat_target.vout_method = 0;
        ...
        fprintf(stderr, "plat_sdl_screen changeded by w:%d, h:%d\n", win_w,win_h);
    }
  }
  ...
  if (plat_target.vout_method == 0) {
    ...
    ScreenSurface = SDL_SetVideoMode(240,160, 16, SDL_HWSURFACE);
    if (ScreenSurface == NULL) {
      fprintf(stderr, "SDL_SetVideoMode failed(ScreenSurface): %s\n", SDL_GetError());
      return -1;
    }
...
int plat_sdl_init(void)
{
  ...
  printf("plat_sdl: g_menuscreen_w:%d, g_menuscreen_h:%d\n",g_menuscreen_w, g_menuscreen_h); //added for test
  ...
  //ScreenSurface = SDL_SetVideoMode(320, 480, 0, plat_sdl_screen->flags);
    ScreenSurface = SDL_SetVideoMode(240, 160, 16, SDL_SWSURFACE ); //for RS-90
  if (ScreenSurface==NULL){
    fprintf(stderr, "SDL_SetVideoMode(ScreenSurface) failed: %s\n", SDL_GetError());
    goto fail;
  }
  //plat_sdl_screen = SDL_CreateRGBSurface(SDL_SWSURFACE, g_menuscreen_w, g_menuscreen_h, 0, 0, 0, 0, 0);
  plat_sdl_screen = SDL_CreateRGBSurface(SDL_SWSURFACE,320, 240, 0, 0, 0, 0, 0);
  if (plat_sdl_screen == NULL) {
    fprintf(stderr, "SDL_SetVideoMode failed: %s\n", SDL_GetError());
    goto fail;
  }
  else{
        fprintf(stderr, "plat_sdl_screen initialized by w:%d, h:%d\n",\
